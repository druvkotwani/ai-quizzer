      ...
      {cards.map((card) => (
        ...
        >
          <div className="max-w-4xl mx-auto text-center px-4 w-full">
            ...
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-3 md:mb-6">{card.title}</h2>
            <p className="text-base md:text-xl mb-6 md:mb-12 max-w-2xl mx-auto px-2">{card.description}</p>
          ...
          {cards.map((card, index) => (
            <button
              ...
              className={`w-2 md:w-3 h-2 md:h-3 rounded-full transition-all ${
                index === activeCardIndex ? 'bg-white w-6 md:w-8' : 'bg-gray-400'
              }`}
              onClick={() => {
                window.scrollTo({
